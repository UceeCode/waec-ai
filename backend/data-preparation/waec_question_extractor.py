...
class WAECQuestionExtractor:
    ...
    def _clean_text(self, text: str) -> str:
        ...
        instructional_line_patterns = [
            re.compile(r'.*WAEC\s*(?:Past\s*Questions)?\s*(?:for\s+.*?)?\s*-\s*Uploaded\s*on\s*(?:https?://)?(?:www\.)?myschoolgist\.com.*', re.IGNORECASE),
            re.compile(r'.*(?:www\.)?myschoolgist\.com.*', re.IGNORECASE),
            re.compile(r'.*myschoolgist\.com.*', re.IGNORECASE), 
            re.compile(r'^\s*P\d+\s*(?:Nov\.)?|\d{4}\s*W\.A\.S\.S\.C\.E\.\s*\d{4}.*', re.IGNORECASE),
            re.compile(r'.*Name:.*', re.IGNORECASE),
            re.compile(r'.*Identification Number:.*', re.IGNORECASE),
            re.compile(r'.*THE WEST AFRICAN EXAMINATIONS COUNCIL.*', re.IGNORECASE),
            re.compile(r'.*West African Senior School Certificate Examination.*', re.IGNORECASE),
            re.compile(r'.*(?:November|June|May|Aug)\s*\d{4}.*', re.IGNORECASE),
            re.compile(r'.*GEOGRAPHY\s+\d.*|AGRICULTURAL\s+SCIENCE\s+\d.*|COMMERCE\s+\d.*|CHRISTIAN\s+RELIGIOUS\s+KNOWLEDGE\s+\d.*|LITERATURE\s+IN\s+ENGLISH\s+\d.*|ENGLISH\s+LANGUAGE\s+\d.*', re.IGNORECASE), 
            re.compile(r'.*Human\s+and\s+Regional.*|Objective\s+Test|Essay\s+Test|Drama\s+and\s+Poetry.*', re.IGNORECASE), 
            ...
            re.compile(r'.*The\s+West\s+African\s+Examinations\s+Council.*', re.IGNORECASE),
